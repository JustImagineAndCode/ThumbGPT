...
import { ChevronDownIcon, CpuIcon, Dice1, ImageIcon, PenToolIcon, SparkleIcon, SquareIcon } from 'lucide-react';
import { div, style } from 'motion/react-client';

function StyleSelector( {value , onChange , isOpen,setIsOpen} : {value: ThumbnailStyle; onChange:(style: ThumbnailStyle) => void;
  ...
  return (
    <div className='relative space-y-3 dark'>
        ...
        <label className=' text-sm block font-medium text-zinc-200'> Thumbnail Style</label>
        <button type ='button' onClick={()=> setIsOpen(!isOpen)} className='flex w-full items-center 
        justify-between rounded-md border px-4 py-3 
